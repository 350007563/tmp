...
from core.data.data_utils import img_feat_path_load, img_feat_load, ques_load, ans_stat
from core.data.data_utils import proc_ques, proc_ans
...
class DataSet(Data.Dataset):
    def __init__(self, __C):
        ...
        if self.__C.PRELOAD:
            ...
            self.iid_to_img_feat = img_feat_load(self.img_feat_path_list, self.__C)
            time_end = time.time()
    ...
    def __getitem__(self, idx):
        ...
        if self.__C.RUN_MODE in ['train']:
            ...
            if self.__C.PRELOAD:
                ...
                assert img_feat_x.shape == (self.__C.IMG_FEAT_GRID, self.__C.IMG_FEAT_GRID, self.__C.IMG_FEAT_SIZE)
                img_feat_x = img_feat_x.reshape((-1, self.__C.IMG_FEAT_SIZE))
            img_feat_iter = img_feat_x

            # Process question
            ques_ix_iter = proc_ques(ques, self.__C)
            ...
            # Process question
            ques_ix_iter = proc_ques(ques, self.__C.MAX_TOKEN)
