...
def img_feat_load(path_list, __C):
    iid_to_feat = {}
    ...
    for ix, path in enumerate(path_list):
        ...
        assert img_feat_x.shape == (__C.IMG_FEAT_GRID, __C.IMG_FEAT_GRID, __C.IMG_FEAT_SIZE)
        img_feat_x = img_feat_x.reshape(-1, __C.IMG_FEAT_SIZE)
...
def proc_ques(ques, __C):
    max_token = __C.MAX_TOKEN
    words = re.sub(
        r"([.,'!?\"()*#:;])",
        '',
        ques['question'].lower()
    ...
    tokenizer = AutoTokenizer.from_pretrained(__C.BERT_VERSION)
